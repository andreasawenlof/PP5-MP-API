...
class ReviewHistorySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'review', 'editor', 'editor_username',
                  'edited_at', 'updated_feedback', 'revision_details']
    ...
    def get_revision_details(self, obj):
        ...
        if request and request.user.profile.is_composer:
            return {
                'number': obj.revision_number,
                'feedback': obj.updated_feedback
            }
        return {
            'feedback': obj.updated_feedback
        }
...
class ReviewSerializer(serializers.ModelSerializer):
    ...
    track_genre = serializers.CharField(
        source="track.genre.name", read_only=True)
    track_mood = serializers.CharField(
        source="track.mood.name", read_only=True)
    ...
    class Meta:
        model = Review
        fields = [
            'id', 'track', 'track_title', 'track_status', 'track_genre', 'track_mood', 'track_project_type',
            'reviewer', 'reviewer_username', 'reviewer_avatar', 'feedback',
            'status', 'reviewed_at', 'review_history'
        ]
    ...
    def validate_track(self, value):
        if value.status != 'ready_for_review':
            raise serializers.ValidationError(
                "This track is not ready for review.")
        return value
